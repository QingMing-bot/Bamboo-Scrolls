...
func (e *Executor) Exec(ctx context.Context, user, addr, key, cmd string, timeout time.Duration) (string, string, int, error) {
	if user == "" || addr == "" {
	...
	// 获取/建立连接
	client, err := e.pool.Get(user, addr, key)
	if err != nil {
		return "", "", -1, err
	}
...
func (p *ConnectionPool) Get(user, addr, key string) (*gssh.Client, error) {
	...
	signer, err := gssh.ParsePrivateKey([]byte(key))
	if err != nil {
		return nil, fmt.Errorf("parse key: %w", err)
	}
	conf := &gssh.ClientConfig{User: user, Auth: []gssh.AuthMethod{gssh.PublicKeys(signer)}, HostKeyCallback: gssh.InsecureIgnoreHostKey(), Timeout: 10 * time.Second}
