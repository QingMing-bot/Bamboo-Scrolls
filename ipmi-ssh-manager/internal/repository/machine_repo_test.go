...
func openMemMachines(t *testing.T) *sql.DB {
	...
	_, err = db.Exec(`CREATE TABLE machines( id INTEGER PRIMARY KEY AUTOINCREMENT, ipmi_ip TEXT UNIQUE, ssh_ip TEXT, ssh_user TEXT, ssh_key TEXT, remark TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP );`)
	if err != nil {
		t.Fatal(err)
	}
	return db
