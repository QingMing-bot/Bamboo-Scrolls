...
type SSHExecutor interface {
	Exec(ctx context.Context, user, addr, key, cmd string, timeout time.Duration) (stdout, stderr string, exitCode int, err error)
}
...
type ExecService struct {
	...
	maxParallel int // <=0 表示不限制
	mu          sync.Mutex
	jobs        map[string]context.CancelFunc // jobID -> cancel
}

func NewExecService(repo *repository.MachineRepo, writer *HistoryWriter, executor SSHExecutor, maxParallel int) *ExecService {
...
func (s *ExecService) BatchExec(task domain.ExecTask) ([]domain.ExecResult, error) {
	...
	for _, id := range task.MachineIDs {
		...
		go func(mc domain.Machine) {
			...
			stdout, stderr, code, exErr := s.executor.Exec(ctx, mc.SSHUser, mc.SSHIP, mc.SSHKey, task.Command, timeout)
			finish := time.Now()
			r := domain.ExecResult{
...
func (s *ExecService) StreamExecWithCtx(ctx context.Context, task domain.ExecTask, cb func(domain.ExecResult)) error {
	...
	for _, id := range task.MachineIDs {
		...
		go func(m domain.Machine) {
			...
			defer cancel()
			stdout, stderr, code, exErr := s.executor.Exec(cctx, m.SSHUser, m.SSHIP, m.SSHKey, task.Command, timeout)
			finish := time.Now()
			res := domain.ExecResult{MachineID: int64(m.ID), IPMIIP: m.IPMIIP, SSHIP: m.SSHIP, SSHUser: m.SSHUser, Stdout: stdout, Stderr: stderr, ExitCode: code, Err: exErr}
			cb(res)
			if s.hWriter != nil {
